...
from .theme import apply_theme, get_condition_color
from ..config import NAVY, GOLD, COLOR_GOOD, COLOR_BAD, COLOR_MODERATE, COLOR_NEUTRAL
from ..data.models import SurveyRecord
...
def chart_map(records: list[SurveyRecord], title: str = "خريطة مواقع المسح") -> BytesIO:
    ...
    if no_building:
        ...
        ax.scatter(lngs, lats, c=NAVY, s=15, alpha=0.4, label=ar("بدون مبنى"), zorder=2)
    ...
    try:
        ...
        if len(no_build_web) > 0:
            ax.scatter(
                ...
                c=NAVY, s=15, alpha=0.4, label=ar("بدون مبنى"), zorder=2
            )
