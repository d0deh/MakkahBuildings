...
from .theme import apply_theme, get_chart_colors, get_condition_color, get_finish_color
        ...
        if value_labels:
            for bar, val in zip(bars, values):
                ax.text(bar.get_width() + max(values) * 0.02, bar.get_y() + bar.get_height() / 2,
                        str(val), ha='left', va='center', fontweight='bold', color=NAVY, fontsize=10)
    else:
        x_pos = np.arange(len(labels))
        bars = ax.bar(x_pos, values, color=colors, width=0.6, edgecolor='white', linewidth=0.5)
        ax.set_xticks(x_pos)
        ...
        if value_labels:
            for bar, val in zip(bars, values):
                ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + max(values) * 0.02,
                        str(val), ha='center', va='bottom', fontweight='bold', color=NAVY, fontsize=10)

    ax.set_title(ar(title), fontsize=16, fontweight='bold', color=NAVY, pad=15)
...
def chart_building_presence(stats: AreaStatistics) -> BytesIO:
    ...
    return _bar_chart(
        ...
        colors=[GOLD, NAVY, "#78909C"][:len(data)],
    )
